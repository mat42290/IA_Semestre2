...
heuristique2(U, H) :-
    final_state(Fin),
    findall(D,
            (coordonnees(C1, U, P), coordonnees(C2, Fin, P), distance(C1, C2, D), D>0, P\=vide),
            L),
    sumlist(L,H).

% Test des heuristiques :

testheuristique1(H1,H2) :-
    initial_state(Ini),
    final_state(Fin),
    heuristique1(Ini,H1), % on attend H1 = 4
    heuristique1(Fin,H2). % on attend H2 = 0

testheuristique2(H1,H2) :-
    initial_state(Ini),
    final_state(Fin),
    heuristique2(Ini,H1), % on attend H1 = 5
    heuristique2(Fin,H2). % on attend H2 = 0
